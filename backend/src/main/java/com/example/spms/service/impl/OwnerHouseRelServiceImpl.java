package com.example.spms.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.example.spms.model.po.OwnerHouseRel;
import com.example.spms.service.OwnerHouseRelService;
import com.example.spms.mapper.OwnerHouseRelMapper;
import org.springframework.stereotype.Service;

/**
* @author poker
* @description 针对表【owner_house_rel(业主房屋关联表)】的数据库操作Service实现
* @createDate 2026-07-03 15:19:38
*/
@Service
public class OwnerHouseRelServiceImpl extends ServiceImpl<OwnerHouseRelMapper, OwnerHouseRel>
    implements OwnerHouseRelService{

}




