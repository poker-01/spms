  ...
  <div class="manage-page">
    <div class="manage-page__header">
      ...
      <div class="manage-page__actions">
        <button class="btn btn-success" type="button" @click="openAutoGenerateDialog">
          📅 一键生成月度账单
        </button>
    ...
    <!-- 一键生成月度账单弹窗 -->
    <div v-if="autoGenerateDialog.visible" class="dialog-overlay" @click.self="autoGenerateDialog.visible = false">
      <div class="dialog dialog--generate">
        <h3 class="dialog__title">一键生成月度账单</h3>
        <div class="dialog__body">
          <div class="form-field">
            <label class="form-label">账单周期 <span class="form-required">*</span></label>
            <input v-model="autoGenerateDialog.billPeriod" class="form-input" type="month" />
          </div>
          <div class="generate-hint">
            📌 将为当前小区所有业主、所有启用的费用项目自动生成当月账单。<br/>
            已存在账单的房屋+项目组合不会重复生成（每个小区每月只能生成一次）。<br/>
            费用按计算方式自动计算：固定金额、按面积、按户。
          </div>
        </div>
        <div class="dialog__footer">
          <button class="btn btn-ghost" type="button" @click="autoGenerateDialog.visible = false">取消</button>
          <button class="btn btn-success" type="button" :disabled="autoGenerateDialog.submitting" @click="handleAutoGenerate">
            {{ autoGenerateDialog.submitting ? '生成中...' : '确认生成' }}
          </button>
        </div>
      </div>
    </div>
...
const autoGenerateDialog = reactive({
  visible: false,
  billPeriod: '',
  submitting: false,
})
...
const openAutoGenerateDialog = () => {
  ...
  autoGenerateDialog.billPeriod = `${y}-${m}`
  autoGenerateDialog.submitting = false
  autoGenerateDialog.visible = true
}

const handleAutoGenerate = async () => {
  if (!autoGenerateDialog.billPeriod) { alert('请选择账单周期'); return }

  autoGenerateDialog.submitting = true
  try {
    const { data } = await autoGenerateMonthlyBills(autoGenerateDialog.billPeriod)
    autoGenerateDialog.visible = false
    loadData()
    loadStatistics()
    if ((data ?? 0) === 0) {
      alert('该小区当月账单已全部生成，无需重复生成')
    } else {
      alert(`月度账单生成成功！共生成 ${data} 条账单`)
    }
  } catch {
    alert('生成失败')
  } finally {
